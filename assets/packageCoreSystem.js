  ...
  function validatePackageSet(manifests) {
    ...
    for (const manifest of manifests) {
      ...
      if (itemErrors.length) {
        errors.push(...itemErrors.map((msg) => `${normalized.id || "unknown"}: ${msg}`));
      }
      warnings.push(...itemWarnings.map((msg) => `${normalized.id || "unknown"}: ${msg}`));
  ...
  function updateRuntimeState() {
    if (window.Runtime?.updateRuntimeState) {
      window.Runtime.updateRuntimeState({
        ...
        packageCapabilities: Array.from(new Set((installedPackages || []).flatMap((item) => item.manifest?.capabilities || []))),
        packageRoutes: Array.from(new Set((installedPackages || []).flatMap((item) => item.manifest?.routes || [])))
      });
    }
  }
